Description="ts-react-redux-simple-webpack-hmr"
ProjectName=${0%.*}
...
echo "本版只是redux的简版，不安装react-redux，简要演示如何从store获取数据并更新数据"
echo "直接从组件中调用store是反redux的，所以本版代码仅供redux初学者学习使用"
# 主要用于生成绝对路径
yarn add path
...
cat > app.tsx << EOF
import * as React from "react"
import {store, addScore, desScore} from "./store";
...
export const App = () => {
  ...
  const onDesScore = () => {
    store.dispatch(desScore)
    setScore(store.getState().score)
  }
  return (
    <div>
      <h1>your score is: {score}</h1>
      <button onClick={onAddScore}>ADD</button>
      <button onClick={onDesScore}>DES</button>
    </div>
  )
}
